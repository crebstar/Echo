...
int main( int argc, char** argv ) {
	...
	switch( networkAppType ) {
	case TYPE_CLIENT_UDP: {
			
			UDPClient udpProtocolClient( IPAddressReq, PortNumberReq );
			udpProtocolClient.initializeAndRun();
		}
		break;

	case TYPE_CLIENT_TCP: {
			
			TCPClient tcpProtocolClient( IPAddressReq, PortNumberReq );
			tcpProtocolClient.initializeAndRun();
		}
		break;

	case TYPE_SERVER_UDP: {

			UDPServer udpProtocolServer( IPAddressReq, PortNumberReq );
			udpProtocolServer.initializeAndRun();
		} 
		break;

	case TYPE_SERVER_TCP: {

			TCPServer tcpProtocolServer( IPAddressReq, PortNumberReq );
			tcpProtocolServer.initializeAndRun();
		}
		break;

	default:

		printf( "Warning... Could not detect a proper configuration from the command line arguments." );
		break;
	}
